...
module WSDL;
  module Document
    class TestRPC < Test::Unit::TestCase
      ...
      #   ret = @client.echo(echo)
      #   #
      #   now1str = XSD::XSDDateTime.new(now1.utc).to_s
      #   now2str = XSD::XSDDateTime.new(now2.utc).to_s
      #   assert_equal("mystr<>ing2", ret.struct1.m_string)
      ...
      #   t1 = Time.parse('2001-03-17T19:47:31 +01:00')
      #   t2 = Time.parse('2002-03-17T19:47:32 +02:00')


      #   struct1.add(SOAPElement.new('m_datetime', t1.to_s))
      #   struct1.extraattr['m_attr'] = 'myattr1'
      #   echo.add(struct2 = SOAPElement.new('struct-2'))
      #   struct2.add(SOAPElement.new('m_string', 'mystring2'))
      #   struct2.add(SOAPElement.new('m_datetime', t2.to_s))
      #   struct2.extraattr['m_attr'] = 'myattr2'
      #   ret = @client.echo(echo)
      #   # BTW -  it swaps args

      #   assert_equal('mystring2', ret.struct1.m_string)
      #   assert_equal(t2.to_s, ret.struct1.m_datetime.to_s)
      #   assert_equal("mystring1", ret.struct_2.m_string)
      #   assert_equal(t1.to_s, ret.struct_2.m_datetime.to_s)
      #   assert_equal('attr_str<>ing', ret.xmlattr_attr_string)
      #   assert_equal(5, ret.xmlattr_attr_int)

      #   echo = {'struct1' => {'m_string' => 'mystring1', 'm_datetime' => t1.to_s}, 
      #     'struct_2' => {'m_string' => 'mystring2', 'm_datetime' => t2.to_s}}
      #   ret = @client.echo(echo)

      #   assert_equal('mystring2', ret.struct1.m_string)
      #   assert_equal(t2.utc.to_s, ret.struct1.m_datetime.to_s)
      #   assert_equal("mystring1", ret.struct_2.m_string)
      #   assert_equal(t1.utc.to_s, ret.struct_2.m_datetime.to_s)
      # end
...
<foo attr-int="5" attr_string="attr_string">
  <struct1 m_attr="myattr1">
    ...
    <m_datetime>#{t1.xmlschema}</m_datetime>
  </struct1>
  <struct-2 m_attr="myattr2">
    <m_string>mystring2</m_string>
    <m_datetime>#{t2.xmlschema}</m_datetime>
  </struct-2>
</foo>
__XML__
        ret = @client.echo(echo)
        assert_equal('mystring2', ret.struct1.m_string)
        assert_equal(t2.utc.to_s,ret.struct1.m_datetime.utc.to_s)
        assert_equal("mystring1", ret.struct_2.m_string)
        assert_equal(t1.utc.to_s,ret.struct_2.m_datetime.utc.to_s)
